-- Fix search_path security issue for calendar_settings updated_at trigger function
...
DROP TRIGGER IF EXISTS update_calendar_settings_updated_at_trigger ON calendar_settings;
DROP FUNCTION IF EXISTS update_calendar_settings_updated_at();
...
CREATE TRIGGER update_calendar_settings_updated_at_trigger
  BEFORE UPDATE ON calendar_settings
  FOR EACH ROW
  EXECUTE FUNCTION update_calendar_settings_updated_at();

-- Add comment explaining the security fix
COMMENT ON FUNCTION update_calendar_settings_updated_at() IS 
